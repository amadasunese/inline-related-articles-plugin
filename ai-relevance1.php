...
function ira_get_related_posts_html($post_id) {
    ...
    // Get categories of the current post once to avoid repeated DB calls
    ...
    foreach ($query->posts as $p) {
        ...
        if ($use_ai) {
            ...
            // FIX: Correctly check if the looped post shares categories with the original post
...
function ira_similarity_score($text1, $text2) {
    // str_word_count is heavy; use only on small chunks or cache the result if possible
